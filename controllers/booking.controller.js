    ...
    try {
        ...
        if (compName) {
            ...
            for (let i = 0; i < names.length; i++) {
                ...
                if (ageNumeric <= 1) {
                    ageLabel = "INFANT";
                } else if (ageNumeric >= 2 && ageNumeric <= 12) {
                    ageLabel = "KID";
                }
                ...
                companionsList.push({
                    ...
                    age: ageLabel, // Saves "ADULT", "KID", or "INFANT"
                    gender: genders[i],
                    contact: contacts[i],
                    idImage: compIdPath
                });
...
exports.postBooking = async (req, res) => {
    ...
    try {
        const { 
            ...
            guestPhone, guestDateOfBirth, specialRequests, extraFee, 
...
exports.postEditBooking = async (req, res) => {
    try {
        ...
        // Find the booking
        const booking = await Booking.findById(bookingId || req.params.id).populate('user');
        if (!booking) return res.status(404).send('Booking not found');

        // --- 2. UPDATE DATES ---
        if (newCheckIn && newCheckOut) { 
            // Replace space with 'T' to ensure Node.js parses the time correctly
            const newStart = new Date(newCheckIn.replace(' ', 'T'));
            const newEnd = new Date(newCheckOut.replace(' ', 'T'));

            // Optional: Conflict check (ensure the room isn't taken by someone else for new dates)
            const conflict = await Booking.findOne({
                _id: { $ne: booking._id }, // Ignore this current booking
            ...
            if (conflict) {
                return res.send(`
                    <script>
                        alert("Error: The room is already booked for the selected dates.");
                        window.history.back();
                    </script>
                `);
            }
        ...
        // 3. Update Primary Guest Profile
        if (booking.user) {
            const user = await User.findById(booking.user._id);
            if (user) {
                user.firstName = guestFirstName || user.firstName;
                user.middleName = guestMiddleName || user.middleName;
                user.lastName = guestLastName || user.lastName;
                if (guestDateOfBirth && guestDateOfBirth !== "") {
                    user.dateOfBirth = new Date(guestDateOfBirth);
                }
                await user.save();
            }
        }

        // 4. Currency and Payment Logic
        const totalAmount = newTotalPrice ? Number(String(newTotalPrice).replace(/[^0-9.-]/g, '')) : booking.totalPrice;
        const downpayment = amountPaid ? Number(String(amountPaid).replace(/[^0-9.-]/g, '')) : booking.amountPaid;
        const minRequired = totalAmount * 0.5;

        if (status !== 'confirmed' && downpayment < minRequired) {
            return res.send(`
                <script>
                alert("Error: Downpayment must be at least 50% (₱${minRequired.toLocaleString()}). Update NOT saved.");
                window.history.back();
                </script>
            `);
        }
        ...
        if (guestDateOfBirth && guestDateOfBirth !== "") {
            booking.guestDateOfBirth = new Date(guestDateOfBirth);
        }
        ...
        if (req.files && req.files.length > 0) {
            const primaryFile = req.files.find(f => f.fieldname === 'primaryIdImage');
            if (primaryFile) {
                booking.guestIdImage = '/uploads/' + primaryFile.filename;
            }
        }

        // 7. Process Companions (With Date Fix for PDF)
        let companionsList = [];
        if (compName) {
            const names = Array.isArray(compName) ? compName : [compName];
            const ages = Array.isArray(compAge) ? compAge : [compAge];
            ...
            for (let i = 0; i < names.length; i++) {
                if (names[i] && names[i].trim() !== "") {
                    const existingImg = (booking.companions && booking.companions[i]) ? booking.companions[i].idImage : '';
                    const newFile = req.files ? req.files.find(f => f.fieldname === `compIdImage_${i}`) : null;
                    const idImagePath = newFile ? '/uploads/' + newFile.filename : existingImg;

                    const ageNumeric = parseInt(ages[i]) || 0;
                    let ageLabel = ageNumeric <= 1 ? "INFANT" : (ageNumeric <= 12 ? "KID" : "ADULT");
                    ...
                    companionsList.push({
                        ...
                        gender: genders[i] || '',
                        contact: contacts[i] || '',
                        dateOfBirth: (dobs[i] && dobs[i] !== "") ? new Date(dobs[i]) : null,
                        idImage: idImagePath
                    });
                }
            }
        ...
        booking.markModified('companions');
...
exports.downloadBookingPDF = async (req, res) => {
    try {
        ...
        const drawDoubleRow = (label1, value1, label2, value2, yPos) => {
