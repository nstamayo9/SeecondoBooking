...
exports.getAdminDashboard = async (req, res) => {
    try {
        ...
        const bookingsWithBalance = bookings.map(b => {
            ...
            bookingObj.balance = Math.max(0, (bookingObj.totalPrice || 0) - (bookingObj.amountPaid || 0));
            return bookingObj;
        });        
...
exports.updateBookingDetails = async (req, res) => {
    try {
        const { 
            bookingId, 
            guestFirstName, guestMiddleName, guestLastName, guestEmail,
            guestPhone, specialRequests,
            compName, compAge, compGender, compContact, compDateOfBirth, // <-- Destructured here
        ...
        // FIX: Ensure manual time edits (like 13:00) save correctly without timezone shifting
        booking.checkInDate = new Date(newCheckIn.replace(' ', 'T'));
        booking.checkOutDate = new Date(newCheckOut.replace(' ', 'T'));
        ...
        const minRequired = booking.totalPrice * 0.5;
        
        // Only enforce the 50% rule if the admin is setting the status to 'confirmed'
        if (status === 'confirmed' && booking.amountPaid < minRequired) {
            return res.send(`
                <script>
                    alert("Error: To CONFIRM this booking, the downpayment must be at least 50% (₱${minRequired.toLocaleString()}). Update NOT saved.");
                    window.history.back();
                </script>
            `);
        }
        ...
        for (let i = 0; i < names.length; i++) {
            if (names[i] && names[i].trim() !== '') {
                ...
                // FIX: Parse Date of Birth safely
                const rawDob = dobs[i];
                const formattedDob = (rawDob && rawDob.trim() !== "") ? new Date(rawDob) : null;

                // Added categorization logic to match your PDF requirements (ADULT, KID, INFANT)
                const ageNumeric = parseInt(ages[i]) || 0;
                let ageLabel = "ADULT";
                if (ageNumeric <= 1) ageLabel = "INFANT";
                else if (ageNumeric >= 2 && ageNumeric <= 12) ageLabel = "KID";

                companionsList.push({
                    name: names[i],
                    age: ageLabel, // Saves the label to keep PDF consistent
                    dateOfBirth: formattedDob, // <-- Saves the properly formatted Date
...
/**
 * @desc    Get Calendar Resources (Rooms)
 * @route   GET /admin/calendar-resources
 */
exports.getCalendarResources = async (req, res) => {
    try {
        ...
        console.error("Calendar Resource Error:", error);
        res.status(500).json([]);
    }
};

/**
 * @desc    Get Calendar Events (Bookings)
 * @route   GET /admin/calendar-events
 */
exports.getCalendarEvents = async (req, res) => {
    try {
        // 1. Define Date Ranges (FullCalendar usually sends 'start' and 'end' as query params)
        // If they don't exist, we default to the current year
        const startRange = req.query.start ? new Date(req.query.start) : new Date(new Date().getFullYear(), 0, 1);
        const endRange = req.query.end ? new Date(req.query.end) : new Date(new Date().getFullYear(), 11, 31);

        // 2. Fetch bookings excluding cancelled ones
        const bookings = await Booking.find({ 
            status: { $ne: 'cancelled' },
            checkInDate: { $lte: endRange },
            checkOutDate: { $gte: startRange }
        })
        .populate('room')
        .populate('user');
        ...
        let events = bookings.map(b => {
            ...
            // If check-out is May 5, it stops at May 4 11:59PM. We add 1 day to show it covering May 5.
            end.setDate(end.getDate() + 1); 

            const guestName = b.user 
                ? `${b.user.firstName || ''} ${b.user.lastName || ''}`.trim() 
                : 'Guest';
                
            const shortId = b._id.toString().slice(0,6).toUpperCase();
            
            return {
                id: b._id.toString(),
                resourceId: b.room._id.toString(),
                start: start.toISOString(),
                end: end.toISOString(),
                title: `#${shortId} - ${guestName}`,
                // IMPORTANT: We remove the 'color' property here because 
                // the frontend CSS classes now handle the 'Soft Badge' look.
                allDay: true,
                extendedProps: {
                    bookingId: b._id,
                    guestName: guestName,
                    shortId: shortId,
                    status: b.status, // used by frontend 'eventClassNames'
        ...
        promos.forEach(p => {
            if (p.eligibleDates && p.eligibleDates.length > 0) {
                p.eligibleDates.forEach(dateStr => {
                    ...
                    // Check if promo date falls within the visible calendar range
                    if (d >= startRange && d <= endRange) {
                        events.push({
                            title: `PROMO: ${p.code}`,
                            start: dateStr,
                            display: 'background',
                            backgroundColor: 'rgba(32, 201, 151, 0.15)', // Light Teal Background
                            classNames: ['bg-promo-vertical'],
                        });
                    }
                });
        ...
        // Ensure you have 'date-holidays' package installed
        ...
        holidays.forEach(h => {
            ...
            if (hDate >= startRange && hDate <= endRange) {
                events.push({
                    ...
                    start: h.date,
                    allDay: true,
                    display: 'background', // Changed to background so it doesn't clutter room rows
                    classNames: ['bg-holiday-vertical'],
                    backgroundColor: 'rgba(111, 66, 193, 0.1)' // Very faint purple
                });
            }
        });
